...
from wavekit import Signal, VcdReader, Waveform  # noqa: E402
from wavekit.readers.vcd.vcd_parser import VCDParser, _VCDResourceError  # noqa: E402
...
t(
    ...
    lambda: (
        (
            ...
            ok('parse jtag header', f'{len(p.signals)} sigs, ts={p.ts_sec:.1e}s'),
        )[0]
        if False
        else None
    ),
...
def _generate_and_test(example_name, tb_file, vcd_name):
    ...
    r = subprocess.run(
        ['iverilog', '-g2012', '-o', str(vvp)] + [str(f) for f in sv_files],
        cwd=str(sv_path.parent),
        capture_output=True,
        text=True,
    )
    if r.returncode != 0:
    ...
    # Run
    r = subprocess.run(['vvp', str(vvp)], cwd=str(sv_path.parent), capture_output=True, text=True)
    if r.returncode != 0:
...
if shutil.which('iverilog') and shutil.which('vvp'):
    ...
    t(
        'fifo_latency VCD',
        lambda: _generate_and_test('fifo_latency', 'fifo_tb.sv', 'fifo_latency_tb.vcd'),
    )
    t(
        'fifo_occupancy VCD',
        lambda: _generate_and_test('fifo_occupancy', 'fifo_tb.sv', 'fifo_occupancy_tb.vcd'),
    )
else:
